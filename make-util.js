...
var getCommonPackInfo = function(modOutDir) {
  ...
  if (!packageJson || !packageJson.name || !packageJson.version) {
    fail(
      `The common module's package.json must define a name and version: ${packageJsonPath}`
    );
  }
...
var buildNodeTask = function(taskPath, outDir) {
  ...
  if (test("-f", packageJsonPath)) {
    ...
    if (
      ...
      fail(
        "The package.json should not contain dev dependencies. Move the dev dependencies into a package.json file under the Tests sub-folder. Offending package.json: " +
          packageJsonPath
      );
    }
...
var matchFind = function(pattern, root, options) {
  ...
  if (noRecurse) {
    ...
    items = find(root).filter(function(item) {
...
var installNode = function(nodeVersion) {
  ...
  if (nodeVersion === run("node -v")) {
    console.log(
      "skipping node install for tests since correct version is running"
    );
    return;
  }
  ...
  switch (platform) {
    case "darwin":
      var nodeArchivePath = downloadArchive(
        nodeUrl +
          "/" +
          nodeVersion +
          "/node-" +
      ...
      addPath(
        path.join(nodeArchivePath, "node-" + nodeVersion + "-darwin-x64", "bin")
      );
      break;
    case "linux":
      var nodeArchivePath = downloadArchive(
      ...
      addPath(
        path.join(nodeArchivePath, "node-" + nodeVersion + "-linux-x64", "bin")
      );
      break;
    case "win32":
      ...
      if (!test("-f", marker)) {
        var nodeExePath = downloadFile(
          nodeUrl + "/" + nodeVersion + "/win-x64/node.exe"
        );
        var nodeLibPath = downloadFile(
          nodeUrl + "/" + nodeVersion + "/win-x64/node.lib"
        );
        rm("-Rf", nodeDirectory);
...
var copyGroup = function(group, sourceRoot, destRoot) {
  ...
  if (group.dest && group.dest.indexOf("<CULTURE_NAME>") >= 0) {
    cultureNames.forEach(function(cultureName) {
      ...
      var localizedGroupJson = JSON.stringify(group).replace(
        /<CULTURE_NAME>/g,
        cultureName
      );
  ...
  source = source.map(function(val) {
    // root the paths
...
var removeGroup = function(group, pathRoot) {
  ...
  var rootedItems = group.items.map(function(val) {
    // root the paths
...
var createMarkdownDocFile = function(
  taskJson,
  taskJsonPath,
  docsDir,
  mdDocOutputFilename
  ...
  if (test("-f", iconPath)) {
    var docIconPath = path.join(
      path.dirname(outFilePath),
      "_img",
      cleanString(taskJson.name).toLowerCase() + ".png"
    );
  ...
  fs.writeFileSync(
    outFilePath,
    getTaskMarkdownDoc(taskJson, mdDocOutputFilename)
  );
};
...
var getTaskMarkdownDoc = function(taskJson, mdDocOutputFilename) {
  ...
  taskMarkdown +=
    "# " +
    cleanString(taskJson.category) +
    ": " +
    cleanString(taskJson.friendlyName) +
    os.EOL +
    os.EOL;
  taskMarkdown +=
    "![](_img/" +
    cleanString(taskJson.name).toLowerCase() +
    ".png) " +
  ...
  taskMarkdown +=
    "[!INCLUDE [temp](../_shared/control-options-arguments.md)]" + os.EOL;
...
var getTaskYaml = function(taskJson) {
  ...
  taskYaml +=
    "- task: " + taskJson.name + "@" + taskJson.version.Major + os.EOL;
  ...
  taskJson.inputs.forEach(function(input) {
    ...
    if (input.required && input.visibleRule && input.visibleRule.length > 0) {
      ...
      requiredOrNot +=
        "# Required when " +
        camelize(input.visibleRule)
          .replace(/ = /g, " == ")
          .replace(
            visibleRuleInputNameCamel,
            getAliasOrNameForInputName(taskJson.inputs, visibleRuleInputName)
          );
    }

    // Does the input have a default value?
    var isDefaultValueAvailable =
      input.defaultValue && input.defaultValue.length > 0;
...
// package functions
//------------------------------------------------------------------------------
var linkNonAggregatedLayoutContent = function(
  sourceRoot,
  destRoot,
  metadataOnly
) {
  ...
  fs.readdirSync(sourceRoot).forEach(function(itemName) {
    ...
    fs.readdirSync(taskSourcePath).forEach(function(itemName) {
      ...
      if (
        metadataOnly &&
        metadataFileNames.indexOf(itemName.toUpperCase()) < 0
      ) {
        return;
      }
...
var linkAggregatedLayoutContent = function(
  sourceRoot,
  destRoot,
  release,
  commit,
  ...
  fs.readdirSync(sourceRoot).forEach(function(itemName) {
    ...
    var sourceTask = JSON.parse(
      fs.readFileSync(path.join(taskSourcePath, "task.json"))
    );
    if (
      typeof sourceTask.version.Major != "number" ||
      typeof sourceTask.version.Minor != "number" ||
      typeof sourceTask.version.Patch != "number"
    ) {
      fail(
        `Expected task.version.Major/Minor/Patch to be numbers (${taskSourcePath})`
      );
    }
    ...
    if (!taskDestPath) {
      taskDestPath = path.join(
        destRoot,
        itemName + `__v${sourceTask.version.Major}`
      );
    ...
    if (test("-e", taskDestPath)) {
      ...
      var destTask = JSON.parse(
        fs.readFileSync(path.join(taskDestPath, "task.json"))
      );
      var sourceVersion = `${sourceTask.version.Major}.${
        sourceTask.version.Minor
      }.${sourceTask.version.Patch}`;
      var destVersion = `${destTask.version.Major}.${destTask.version.Minor}.${
        destTask.version.Patch
      }`;
      if (semver.gt(sourceVersion, destVersion)) {
        fail(
          `Expected minor+patch version for task already in the aggregate layout, to be greater or equal than non-aggregated layout being merged. Source task: ${taskSourcePath}`
        );
      }
    } else {
      ...
      fs.writeFileSync(
        taskDestPath + (release ? `_m${release}` : "") + `_${commit}`,
        ""
      );
    }
  });
...
var getNonAggregatedLayout = function(packagePath, release, commit) {
  ...
  var zipPath = path.join(
    localappdata,
    "vsts-tasks",
    `non-aggregated-tasks_m${release}_${commit}.zip`
  );
  var markerPath = `${zipPath}.completed`;
  if (!test("-f", markerPath)) {
    fail(
      `Non-aggregated layout for m${release} (${commit}) not found in the cache. Publish the latest m${release} and then try again.`
    );
  }
  ...
  run(
    `powershell.exe -NoLogo -Sta -NoProfile -NonInteractive -ExecutionPolicy Unrestricted -Command "& '${path.join(
      __dirname,
      "Expand-Tasks.ps1"
    )}' -ZipPath '${zipPath}' -TargetPath '${destPath}'"`,
...
var getRefs = function() {
  ...
  run(
    'git branch --list --remotes "origin/releases/m*"',
    /*inheritStreams*/ false,
    /*noHeader*/ true
  )
    .split("\n")
    .forEach(function(branch) {
      ...
      if (release < 108 || release > (info.head.release || 999)) {
        return;
      }
...
var compressTasks = function(sourceRoot, destPath, individually) {
  ...
  run(
    `powershell.exe -NoLogo -Sta -NoProfile -NonInteractive -ExecutionPolicy Unrestricted -Command "& '${path.join(
      __dirname,
      "Compress-Tasks.ps1"
    )}' -SourceRoot '${sourceRoot}' -TargetPath '${destPath}' -Individually:${
...
var createNonAggregatedZip = function(buildPath, packagePath) {
  ...
  linkNonAggregatedLayoutContent(
    buildPath,
    nestedZipsContentPath,
    /*metadataOnly*/ false
  );
  ...
  console.log();
  console.log(
    "> Creating nested task zips (content for non-aggregated tasks layout)"
  );
  var nonAggregatedLayoutPath = path.join(packagePath, "non-aggregated-layout");
  compressTasks(
    nestedZipsContentPath,
    nonAggregatedLayoutPath,
    /*individually:*/ true
  );
  ...
  fs.writeFileSync(
    path.join(nonAggregatedLayoutPath, "layout-version.txt"),
    "2"
  );
 ...
 */
var createNugetPackagePerTask = function(
  packagePath,
  /*nonAggregatedLayoutPath*/ layoutPath
) {
  console.log();
  ...
  fs.readdirSync(layoutPath).forEach(function(taskFolderName) {
    ...
    var taskVersion =
      taskJsonContents.version.Major +
      "." +
      taskJsonContents.version.Minor +
      "." +
      taskJsonContents.version.Patch;
    var taskName = taskJsonContents.name;

    // Create the full task name so we don't need to rely on the folder name.
    var fullTaskName = `Mseng.MS.TF.DistributedTask.Tasks.${taskName}V${
      taskJsonContents.version.Major
    }`;

    // Create xml entry for UnifiedDependencies
    unifiedDepsContent.push(
      `  <package id="${fullTaskName}" version="${taskVersion}" availableAtDeployTime="true" />`
    );

    // Create xml entry that we need to configure servicing file
    servicingXmlContent.push(
      getServicingXmlContent(taskFolderName, fullTaskName, taskVersion)
    );
    ...
    var taskNuspecPath = createNuspecFile(
      taskZipPath,
      fullTaskName,
      taskVersion
    );
    var taskPublishFolder = createNuGetPackage(
      nugetPackagesPath,
      taskFolderName,
      taskNuspecPath,
      taskZipPath
 ...
 * @param {*} taskVersion
 */
var getServicingXmlContent = function(
  taskFolderName,
  fullTaskName,
  taskVersion
) {
  ...
  servicingXmlContent +=
    `  <Directory Path="[ServicingDir]Tasks\\Individual\\${taskFolderName}\\">` +
    os.EOL;
  servicingXmlContent +=
    `    <File Origin="nuget://${fullTaskName}/*" />` + os.EOL;
...
var createNuspecFile = function(taskLayoutPath, fullTaskName, taskVersion) {
  ...
  contents +=
    "      <requireLicenseAcceptance>false</requireLicenseAcceptance>" + os.EOL;
  contents +=
    "      <description>For VSS internal use only</description>" + os.EOL;
 ...
 */
var createNuGetPackage = function(
  publishPath,
  taskFolderName,
  taskNuspecPath,
  taskLayoutPath
  ...
  run(
    `nuget pack "${taskNuspecPath}" -BasePath "${taskLayoutPath}" -NoDefaultExcludes`,
    /*inheritStreams:*/ true
  );
...
var createPushCmd = function(taskPublishFolder, fullTaskName, taskVersion) {
  ...
  fs.writeFileSync(
    taskPushCmdPath,
    `nuget.exe push ${nupkgName} -source "${taskFeedUrl}" -apikey ${apiKey}`
  );
};
...
var renameFoldersFromAggregate = function renameFoldersFromAggregate(
  pathWithLegacyFolders
) {
  // Rename folders
  fs.readdirSync(pathWithLegacyFolders).forEach(function(taskFolderName) {
    if (
      taskFolderName.charAt(taskFolderName.length - 1) ===
        taskFolderName.charAt(taskFolderName.length - 3) &&
      taskFolderName.charAt(taskFolderName.length - 2) ===
        taskFolderName.charAt(taskFolderName.length - 4)
    ) {
      var currentPath = path.join(pathWithLegacyFolders, taskFolderName);
      var newPath = path.join(
        pathWithLegacyFolders,
        taskFolderName.substring(0, taskFolderName.length - 2)
      );
...
// [stfrance]: remove this once we have fully migrated to nuget package per task.
var generatePerTaskForLegacyPackages = function generatePerTaskForLegacyPackages(
  pathWithLegacyFolders
) {
...
var createHotfixLayout = function(packagePath, taskName) {
  ...
  scriptContent +=
    "Update-DistributedTaskDefinitions -TaskZip $PSScriptRoot\\tasks.zip" +
    os.EOL;
...
var createAggregatedZip = function(packagePath) {
  ...
  var nonAggregatedLayoutPath = path.join(packagePath, "non-aggregated-layout");
  linkAggregatedLayoutContent(
    nonAggregatedLayoutPath,
    aggregatedLayoutPath,
    /*release:*/ "",
    /*commit:*/ refs.head.commit,
    taskDestMap
  );
  ...
  fs.readdirSync(aggregatedLayoutPath) // walk each item in the aggregate layout
    .forEach(function(itemName) {
      ...
      if (typeof task.version.Major != "number") {
        fail(
          `Expected task.version.Major/Minor/Patch to be a number (${taskPath})`
        );
      }
      ...
      if (majorVersions[key]) {
        fail(
          `Tasks GUID + Major version must be unique within the aggregated layout. Task 1: ${
            majorVersions[key]
          }; task 2: ${taskPath}`
        );
      }
  ...
  var aggregatedZipPath = path.join(
    packagePath,
    "pack-source",
    "contents",
    "Microsoft.TeamFoundation.Build.Tasks.zip"
  );
